...
Codex 限额。它可以显示 5 小时或周余量，自动刷新，并复用本机现有的 Codex 登录状态。
...
- 状态栏中心数字表示当前限额视图的剩余百分比。为保证小尺寸下清晰，中心不显示
  `%` 符号。
- 余量弧线使用固定的红 → 黄 → 绿渐变：`0` 为红色、`20` 为黄色、`50–100`
  为绿色。
- 点击指示器可查看两种限额、重置倒计时、最后更新时间、手动刷新和语言选项。
- 首次启动默认使用英文，可即时切换为简体中文；应用会记住语言选择。

## 显示逻辑

- 首次使用优先显示 5 小时余量。切换到周余量后，状态栏立即更新，并在下次启动时
  保留该选择。
- 如果首选限额暂不可用、另一种限额可用，应用会临时显示可用读数，但不会改变首选项。
- 剩余限额按 `floor(100 - usedPercent)` 计算，并限制在 `0...100`。
- 刷新失败时，有效缓存数字会继续显示，但余量环变为灰色。读数到达自身重置时间后
  立即失效；没有有效读数时显示 `—`。
- 重置时间使用相对倒计时，最多显示两个单位，不显示秒。
- 应用会在启动、每 5 分钟、Mac 唤醒、限额变化、切换视图和手动刷新时更新数据。
