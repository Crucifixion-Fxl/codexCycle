...
Codex 5 小时和周余量。它会自动刷新，并复用本机现有的 Codex 登录状态。
...
- 余量弧线使用固定的红 → 黄 → 绿渐变：`0` 为红色、`20` 为黄色、`50–100`
  为绿色。
- 点击指示器可查看 5 小时与周余量、各自的重置倒计时、最后更新时间、“立即刷新”
  和“立即请求”。
- 界面默认跟随 macOS 选择的系统语言，也可在菜单中选择“跟随系统”、English 或
  简体中文。

## 显示逻辑

- 只显示主 `codex` 计量桶中精确的 5 小时和 7 天窗口，其他窗口不会被替代使用。
- 剩余限额按 `floor(100 - usedPercent)` 计算，并限制在 `0...100`。
- 刷新失败时，有效缓存数字会继续显示，但余量环变为灰色。读数到达自身重置时间后
  立即失效；没有有效读数时显示 `—`。
- 重置时间使用相对倒计时，最多显示两个单位，不显示秒。
- 应用会在启动、每 5 分钟、Mac 唤醒、限额变化、任一窗口重置和手动刷新时更新数据。
- 应用每天按本地时间 7:00 发起一次最小、临时、只读的 Codex 请求，以启动或滚动
  5 小时窗口，然后重新读取限额。若当时 Mac 休眠或应用未运行，会在当天下一次唤醒
  或启动后补执行一次。
- “立即请求”会按需运行同一条安全请求，并在完成后刷新限额读数。
...
`account/rateLimits/read`。每日及手动额度刷新请求通过已验证的本地 Runtime，以临时
`codex exec` 会话运行，并启用只读 sandbox、通过指令禁用工具且忽略用户配置。应用
不读取或保存凭据，不直接发起网络请求，不收集分析数据，也不发送遥测；运行 Codex
Runtime 前会验证可执行文件。
