...
final class WeeklyUsageParserTests: XCTestCase {
    private let now = Date(timeIntervalSince1970: 1_800_000_000)

    func testReadsOnlyExactWeeklyWindowFromMainCodexBucket() throws {
        ...
        let reading = try WeeklyUsageParser.parse(response, fetchedAt: now)

        XCTAssertEqual(reading.remainingPercent, 97)
        XCTAssertEqual(reading.resetsAt, Date(timeIntervalSince1970: 1_800_100_000))
        XCTAssertEqual(reading.fetchedAt, now)
    }

    func testRejectsFiveHourWindowInsteadOfSubstitutingIt() {
        let response = GetAccountRateLimitsResult(
            rateLimits: snapshot(id: "codex", used: 12, duration: 300),
            rateLimitsByLimitId: nil
        )

        XCTAssertThrowsError(try WeeklyUsageParser.parse(response, fetchedAt: now)) {
            XCTAssertEqual($0 as? WeeklyUsageError, .noWeeklyWindow)
        }
    }
    ...
    func testRejectsNonCodexBucket() {
        ...
        XCTAssertThrowsError(try WeeklyUsageParser.parse(response, fetchedAt: now)) {
            XCTAssertEqual($0 as? WeeklyUsageError, .noMainCodexBucket)
        }
    }
    ...
    func testFloorsAndClampsRemainingPercentage() throws {
        ...
        XCTAssertEqual(
            try WeeklyUsageParser.parse(fractional, fetchedAt: now).remainingPercent,
            19
        )
        XCTAssertEqual(
            try WeeklyUsageParser.parse(overused, fetchedAt: now).remainingPercent,
            0
        )
    }
